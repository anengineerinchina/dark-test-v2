...
//bitcoindark: custom teleport rpc
Value jl777json(const Array& params, bool fHelp)
{

	if (fHelp || params.size() < 2 || params.size() > 2)
		throw runtime_error(
		"jl777json [{\"requestType\":requestType, \"coin\":coin, \"height\":height, \"minage\":minage, \"M\":M, \"N\":N, \"value\":value, \"totalcrc\":totalcrc},...] <'[\"telepod\",\"telepod\"]'>\n"
		"Send a libjl777 message.");
	RPCTypeCheck(params, list_of(array_type)(array_type));

	Array inputs = params[0].get_array();
    Array telepods = params[1].get_array();

    string requestType = "";
    string coin = requestType;
    string height = coin;
    string minage = height;
    string M = minage;
    string N = M;
    string value = N;
    string totalcrc = value;

	BOOST_FOREACH(Value& input, inputs)
	{
		const Object& o = input.get_obj();

		const Value& requestType_v = find_value(o, "requestType");
		if (requestType_v.type() != str_type)
			throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, missing requestType key");
		requestType = requestType_v.get_str();

		const Value& coin_v = find_value(o, "coin");
		if (coin_v.type() != str_type)
			throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, missing coin key");
		coin = coin_v.get_str();

		const Value& height_v = find_value(o, "height");
		if (height_v.type() != str_type)
			throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, missing height key");
		height = height_v.get_str();

		const Value& minage_v = find_value(o, "minage");
		if (minage_v.type() != str_type)
			throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, missing minage key");
		minage = minage_v.get_str();

		const Value& M_v = find_value(o, "M");
		if (M_v.type() != str_type)
			throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, missing M key");
		string M = M_v.get_str();

		const Value& N_v = find_value(o, "N");
		if (N_v.type() != str_type)
			throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, missing N key");
		N = N_v.get_str();

		const Value& value_v = find_value(o, "value");
		if (value_v.type() != str_type)
			throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, missing value key");
		value = value_v.get_str();

		const Value& totalcrc_v = find_value(o, "totalcrc");
		if (totalcrc_v.type() != str_type)
			throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, missing totalcrc key");
		totalcrc = totalcrc_v.get_str();
	}
	//TODO: call jl777 function
    string finalValue = string("[{")
		...
		+ ", totalcrc:" + totalcrc
		+ "}] {";
	for (unsigned int i = 0; i < telepods.size(); i++)
		finalValue += "telepod: " + telepods[i].get_str() + ", ";
	finalValue += "}";
	return finalValue;
}
